...
func (s *Server) MountHandlers() error {
	...
	mountPublicRoutes := func(prefix string) {
		...
		s.Router.Post(prefix+"/capabilities/discover", api.DiscoverSDKProviders(s.Database))
		s.Router.Post(prefix+"/me/sdk-targets", api.ConfigureSDKTarget(s.Database))
		s.Router.Get(prefix+"/me/sdk-targets", api.SDKTargetControlList(s.Database))
		s.Router.Get(prefix+"/me/routines", api.RoutineDraftList(s.Database))
		s.Router.Get(prefix+"/me/routines/{routineID}", api.RoutineDraftControl(s.Database))
		s.Router.Put(prefix+"/me/routines/{routineID}", api.RoutineDraftControl(s.Database))
		s.Router.Post(prefix+"/me/routines/{routineID}/runs", s.Spaces.RoutineManualRun())
		s.Router.Get(prefix+"/me/routine-runs/{runID}", s.Spaces.RoutineRunControl())
		s.Router.Post(prefix+"/me/routine-runs/{runID}/cancel", s.Spaces.RoutineRunControl())
		s.Router.Delete(prefix+"/me/sdk-targets/{targetID}", api.RevokeSDKTarget(s.Database))
		s.Router.Post(prefix+"/capabilities/targets/resolve", api.ResolveSDKTargets(s.Database))
		s.Router.Post(prefix+"/capabilities/invocations", s.Spaces.InvokeSDKCapability())
		s.Router.Get(prefix+"/capabilities/invocations/{requestID}", s.Spaces.SDKCapabilityResult())
		s.Router.Post(prefix+"/capabilities/invocations/{requestID}/cancel", s.Spaces.CancelSDKCapability())
		s.Router.Get(prefix+"/me/capability-approvals", s.Spaces.SDKCapabilityApprovals())
	...
	s.Router.Post("/internal/agent-runtime/runs/{runID}/routine-wait", s.Spaces.AgentRuntimeRoutineWait())
	s.Router.Post("/internal/agent-runtime/runs/{runID}/routine-agent", s.Spaces.AgentRuntimeRoutineAgent())
